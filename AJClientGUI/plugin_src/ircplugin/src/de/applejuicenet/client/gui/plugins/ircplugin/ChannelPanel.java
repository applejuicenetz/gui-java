...
public class ChannelPanel extends JPanel implements ActionListener {
	...
	private void makePanel() {
			...
			public void keyReleased(KeyEvent ke) {
				...
				if (ke.getKeyCode() == KeyEvent.VK_TAB) {
					...
					if (text.length() > 0) {
						...
						String searchString;
						if (index != -1) {
							searchString = text.substring(index + 1)
									.toLowerCase();
						} else {
						...
						synchronized (usernameList) {
							...
							for (int i = 0; i < users.length; i++) {
								if (users[i].getName().toLowerCase()
				...
				if (me.getClickCount() == 2) {
					...
					if (!user.getName().equalsIgnoreCase(
						...
						for (int i = 1; i < tabbedPane.getTabCount(); i++) {
							if (tabbedPane.getTitleAt(i).equalsIgnoreCase(
									user.getName())) {
								return;
							}
						}
		...
		JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, sp1,
				panel1);
		splitPane
				.setDividerLocation(AppleJuiceDialog.getApp().getSize().width - 200);
				...
				if (message.toLowerCase().compareTo("/ajstats") == 0
					...
					if (message.toLowerCase().compareTo("/ajstats") == 0) {
						if (currentTime > lastStatsPrinted + 60000) {
							...
							parentPanel.parseSendToCommand("PRIVMSG " + name
									+ " :" + text);
						...
						if (currentTime > lastVersionPrinted + 60000) {
							...
							String text = ApplejuiceFassade.getInstance()
									.getVersionInformation().substring(3);
							parentPanel.parseSendToCommand("PRIVMSG " + name
									+ " :" + text);
							updateTextArea(parentPanel.formatNickname("<"
									+ parentPanel.getNickname() + "> ")
	...
	private SimpleAttributeSet writeString(StyledDocument doc,
			SimpleAttributeSet attributes, String toWrite) {
		...
		if (toWrite.length() > 1) {
			while (!parsEnde) {
				try {
					...
					int colorCode = Integer.parseInt(toWrite.substring(index,
							index + 2));
					index += 2;
		...
		try {
			doc.insertString(doc.getLength(), toWrite.substring(index),
					attributes);
		} catch (BadLocationException ex) {
			ex.printStackTrace();
		}
	...
	private void updateCounter() {
		counter.setText(usernameList.getOpCount() + " Ops, "
				+ usernameList.getSize() + " " + counterText);
	}
